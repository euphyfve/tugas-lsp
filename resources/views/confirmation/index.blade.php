...
@section('table-bos')
    <table class="table table-bordered table-dark table-hover" id="tablesigma">
        ...
        <tbody>
            ...
            @if ($trx->isEmpty())
                <td colspan="10" class="text-center">Data Konfirmasi Kosong</td>
            @endif
            ...
            @foreach ($trx as $i)
                <tr>
                    ...
                    <td>
                        <div class="d-flex gap-2 justify-content-center">
                            <a onclick="del({{ $i->id }})" class="btn btn-outline-success">Confirm</i></a>
                        </div>
                    </td>
                </tr>
                <?php $page++; ?>
...
<script>
    ...
    {
        Swal.fire({
        title: "Yakin Ingin Konfirmasi Pembayaran ??",
        icon: "warning",
        ...
        if (result.isConfirmed) {
            window.location.href="{{ url('confirmation/done') }}/" + $id
            Swal.fire({
            title: "Berhasil!",
            text: "Berhasil Mengkonfirmasi",
            icon: "success"
            });
        }
        });
